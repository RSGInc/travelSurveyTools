...
hts_prep_byvar = function(summarize_by = NULL,
  ...
  for (b in seq_along(summarize_by)) {
    ...
    stopifnot(
      "table id not in byvar_dt_v, make id_cols ordered list of ids in each table of data" =
    ...
    if (byvar_is_numeric) {
      byvar_dt_v = hts_bin_var(
        prepped_dt = byvar_dt_v,
        numvar = byvar,
        nbins = 7
      )
    }
  ...
  byvar_dt = Reduce(
    function(x, y) {
      merge(x, y, all.x = FALSE, all.y = FALSE, by = intersect(
        names(x),
        names(y)
      ))
    },
