...
```{r, trip_rate_age, echo=TRUE, fig.width=8, fig.height=6, warning=FALSE}
...
num_trips_employment = hts_summary(
  prepped_dt = DT$num,
  summarize_by = 'employment',
  summarize_var = 'num_trips_wtd',
  summarize_vartype = 'numeric',
...
# Label employment
num_trips_employment_labeled = factorize_df(num_trips_employment,
                                            value_labels,
                                            value_label_colname = 'label')


# Create a plot
ggplot(num_trips_employment_labeled, 
       aes(x = median, y = employment)) + 
